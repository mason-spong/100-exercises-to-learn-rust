...
#[derive(Debug)]
enum TicketNewError {
    TitleError { errorDescription: String },
    DescriptionError { errorDescription: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(error) => match error {
            TicketNewError::TitleError { errorDescription } => {
                panic!("{errorDescription}")
            }
            TicketNewError::DescriptionError { errorDescription } => {
    ...
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError {
                errorDescription: "Title cannot be empty".to_string(),
            });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                errorDescription: "Title cannot be longer than 50 bytes".to_string(),
            });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError {
                errorDescription: "Description cannot be empty".to_string(),
            });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                errorDescription: "Description cannot be longer than 500 bytes".to_string(),
            });
        }
